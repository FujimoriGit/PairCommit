...
- VRT の運用:
  ...
  - **基準画像がまだない `#Preview` は、`ci.yml` が撮って PR に足す。** 比べる相手がいないので、自動で取り込んでも退行を見逃さない。撮られた画像は PR の差分に出るので、そこで見る。`#Preview` の名前を変えたときは新しい名前の画像が足されるだけで、古い名前の画像は残る（消えるのは `update-snapshots` を回したとき）。
