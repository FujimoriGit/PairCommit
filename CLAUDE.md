...
- VRT の運用:
  ...
  - **既存の基準画像との差分は自動で取り込まない。** 意図した変更か退行かはコードから判定できず、自動で取り込むと VRT が何も検出しなくなる。意図した変更のときは、CI が落ちるのを待たず、見た目を変えた push に続けてそのブランチで `update-snapshots` を手動実行する。基準画像がコミットされて返るので、差分をレビューする。落ちてから回すと、次にこのブランチへ触る人は、失敗が自分の変更によるものか前の撮り直し漏れかを、基準画像の画素を調べるまで区別できない。
...
- **1件ずつ妥当性を確かめる。** 該当箇所を読んで主張が成り立つかを見る。直さない判断も理由つきで示す。
- **指摘の行だけを直さない。** 同じ原因の誤りが、同じ関数の別の分岐に残っていることが多い。直す前に、触る関数と呼び出し経路が取りうる分岐を数え上げ、どれも同じ扱いになっているかを見る（削除なら「消せた」「消せなかった」「対象がもう無い」の3つ）。
- **見た目の値は自分で決めない。** 色・寸法を変える指摘は、理由と候補を値付きで出して選んでもらってから実装する。読めるかどうかは比で計算できるが、良く見えるかどうかは計算できない。直すたびに基準画像を撮り直す CI が要るので、選び直しになったときの代償が大きい。
- **「戻せ」「直せ」は直前の対象にそのまま当てる。** 戻す範囲を自分で決め直さない。選択肢を出すときは、言われた言葉をそのまま満たす案を先に置く（自分が置いた制約の内側だけで二択を作らない）。
- **push の前に、自分の差分をレビューする。**
- **レビュースレッドの解決状態に触らない。** 指摘への対応は、返信と修正までで止める。閉じるのは自動レビューの役目で、先に自分で閉じると、そのあと走るレビューは解決済みと見て何もしない。あとから未解決に戻しても、次の push が来るまで誰も閉じないままになる。
