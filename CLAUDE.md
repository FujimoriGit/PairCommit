...
- **1件ずつ妥当性を確かめる。** 該当箇所を読んで主張が成り立つかを見る。直さない判断も理由つきで示す（反対の選択肢では成り立たない理由を書く）。
- **指摘の行だけを直さない。** 指摘は見つかった1箇所を指しているだけで、同じ種類の穴がその関数の別の層に残っていることが多い。直す前に、触る関数と呼び出し経路が取りうる分岐を数え上げ、全部が同じ扱いになっているかを見る（操作そのものの失敗と項目ごとの失敗、成功と失敗と「すでに無い」）。
