...
- 通信の失敗は、利用者が自分で手を打てる単位（iCloud のサインイン・容量・混雑・圏外・近くの端末・接続切れ）に分けて見せ、元のエラーは画面に出さずログ（`Logger`、サブシステム `com.fujimori.PairCommit`）に残す。error レベルは端末に保存されるので、あとから Mac につないで Console.app で読める。
