...
- 予約した催促の取り下げ。予約した通知の文面は予約した時点の状態で作り、配信済みの通知を時刻で取り下げる手段もないので、相手の変更がこの端末に届かないまま時刻が来ると、次にアプリが開かれるかプッシュで起きるまで、もう解消した催促が鳴って通知センターに残る。プッシュの配送は保証されないので、プッシュが届く環境でも残る。
