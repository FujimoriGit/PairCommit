...
- **画面の先頭はビジョン。** active なビジョンは高々1個という不変条件が設計の中心なので、タスクの追加欄と同じ重さの節に置かない。上部の固定カードにして残り日数を出し、タスクはその下に従属させる。期限は任意なので、無いときは同じ場所に「期限なし」と出す。カードは節と違って出さないという選択がなく、空欄にすると設定し忘れなのか意図的なのかが画面から分からない。カードの高さは期限の有無で変えない。
