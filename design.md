...
7. **催促の具体仕様**（ロードマップ7）-> 決定。判定は `PartnershipState.nudges(for:now:)` の純粋関数。
   ...
   - アプリ内表示と端末の通知の両方。相手の変更で飛ぶサイレントプッシュで起きて判定し直す。通知は催促1件につき1つで、識別子を固定して差し替える（同じ催促が続く間に増えない）。解消した催促の通知は取り下げる。
...
- ペアリング結果（ルートレコードのIDと、始めた側かどうか）の保存。どこにも残していないので、起動し直すとロールの選択からになる。
- アプリを開いていない間に催促を出す経路。通知を出しているのは画面の `.task` で、サイレントプッシュで起こされてもそこまで届かない。誰も操作しないまま期限が過ぎただけの催促も、相手の変更がなくプッシュが飛ばないので出ない。
