...
5. **ペアリング時のロール割り当て** -> 済。始める側が自分のロールを選び、そのロールを入れた状態をルートレコードに書いてから共有する。受ける側はMCでロールを受け取るのではなく、参加後に読み込んだ状態から `ownerRole` を読む。MC で渡すのは CKShare の URL だけで済む。
   - 接続したら、まず互いに何を選んだかを送り合う。最初の画面は「どちらで使いますか」と聞くので、2人がそれぞれ自分のロールを選ぶことが多い。ロールが違えば、そのままペアにする。共有は見届ける人を選んだ側が作り、挑む人を選んだ側が受ける。
...
- ペアリング結果（ルートレコードのIDと、始めた側かどうか）は端末に残し、起動し直したら前回の相手につなぎ直す。つながらないだけならペアは残して再試行を出し、ペアが終わっていたときだけ役割の選択に戻す。
