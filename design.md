...
- **役割は画面に出さない。** ロールは固定でスワップもないので、毎画面に出すのは変わらない値を出し続けることになる。どちら側かは操作から分かる ── 承認と差し戻しが並ぶか、完了報告と感情ボタンが並ぶか。役割の語が出るのは、ペアリング前の選択画面だけ。
- **役割はアクセント色で示す。** 見届ける側は藍、挑む側は紫。暖色を使わないのは、感情リアクションが赤・橙・緑を占めているため。役割色をそこへ寄せると、行の色が感情を指しているのか役割を指しているのか読めなくなる。
...
- **フォントは標準のまま。** 独自フォントはバンドルサイズが増えるうえ、VRT のレンダリングが環境で揺れる。
...
9. **ウィジェット** -> 未決。やるかどうかから決めていない。やる場合に出すのは、自分のタスク一覧ではなく相手が待っていること ── 見届ける側には承認待ちの件数、挑む側には進行中のビジョンと残り日数と未完了の件数。アプリを開かなくても相手の存在が見えている状態を作るのが目的なので、一覧の縮小版にすると開かない理由になって逆効果になる。サイズは small 1つ。共有 DB をウィジェットから直に読むのは面倒なので、アプリが App Group にスナップショットを書いてウィジェットはそれを読む。
