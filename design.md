...
- 予約した催促の取り消し。予約した通知の文面は予約した時点の状態で作り、アプリが動いていない間に作り直せるのはプッシュで起きたときだけなので、相手の変更がこの端末に届かないまま時刻が来ると、もう済んだタスクの催促が鳴る。プッシュの配送は保証されないので、署名と capability が入ったあとも起きうる。
- 署名と capability（Team・iCloud コンテナ・Push Notifications・Background Modes の Remote notifications）。プッシュはこれが入るまで配送されない。
- 見た目。「見た目（決定）」のうち、画面を変えることになるもの。アクセント色は標準のまま。ビジョンは `Form` の一節に置かれていて、感情ボタンは行の中の小さな要素。ヒートマップは管理者側にしか出ていない（プレイヤーのタスク行に背景色がない）。
