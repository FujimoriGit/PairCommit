...
6. **片側リセット時の整合性** -> 決定。**リセットは片側だけでは終われない。必ず両者のパートナーシップが終わる。**
   ...
   - 実装はゾーンごと消す。始めた側は private DB のゾーンを、受けた側は shared DB のゾーンを消して共有から抜ける。
   - 相手側は次の取得でレコードが無くなっているので、`pairing` が nil の状態になり最初の画面に戻る。**終了フラグは状態に足さない。** レコードの不在がそのまま「もう無い」を表しているので、増やすと同じことを2通りで表すことになる。
   - 残: リセットを始める入り口（4つのロール画面のどこに置くか）とゾーン削除の実装。相手が消したときに最初の画面へ戻るところまでは入っている。
...
- テストとプレビューの代用品を製品から切り離す構成。`Infrastructure` の中身は `InMemorySynchronizer` だけで、実物の同期層はアプリターゲットにあるのに、アプリがこのモジュールをリンクしている。products から外してテストターゲットだけが依存する形にし、プレビュー用の実装はアプリターゲットに `#if DEBUG` で置く。
