...
5. **ペアリング時のロール割り当て** -> 済。始める側が自分のロールを選び、そのロールを入れた状態をルートレコードに書いてから共有する。受ける側はMCでロールを受け取るのではなく、参加後に読み込んだ状態から `ownerRole` を読む。MC で渡すのは CKShare の URL と、接続直後に送り合う「始める側か受ける側か」だけ。後者がないと、2人とも同じ側を選んだときに接続だけ成立して先へ進まず、画面が止まったままになる。
