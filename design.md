...
1. MCで何を渡すか（CKShare URL or 独自トークン）と、CloudKit Sharingの招待フローとの噛み合わせ
   ...
   -> **実行検証はこれから**: Apple Developer Program は登録手続き中。MC は実機同士でないと繋がらないので、この検証だけは実機2台が要る。
...
- 予約した催促の取り下げ。予約した通知の文面は予約した時点の状態で作り、配信済みの通知を時刻で取り下げる手段もないので、相手の変更がこの端末に届かないまま時刻が来ると、次にアプリが開かれるかプッシュで起きるまで、もう解消した催促が鳴って通知センターに残る。プッシュの配送は保証されないので、署名が入ったあとも残る。
...
**保留**: 実機での検証。Apple Developer Program は登録手続き中で、加入するまで CloudKit とプッシュが使えない。MC はシミュレータ同士では繋がらないので、ペアリングから先は実機2台でしか動かせない。1台だけで使う入り口（`LocalPairing`）は、ペアリングを本物にしたときに落とした。
