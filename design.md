...
- **根の4画面にはナビゲーションタイトルを置かない。** 役割名を消した跡に別の語を入れない。タブも階層もなく、常に4つのうち1画面しか出ないので、見出しは「いまどこにいるか」を教えない。そこから push する画面は戻り先と区別が要るので、見出しを持つ。
...
- **画面の先頭はビジョン。** active なビジョンは高々1個という不変条件が設計の中心なので、タスクの追加欄と同じ重さの節に置かない。上部の固定カードにして残り日数を出し、タスクはその下に従属させる。期限は任意なので、無いときは同じ場所に「期限なし」と出す。カードは節と違って出さないという選択がなく、空欄にすると設定し忘れなのか意図的なのかが画面から分からない。期限が無いと催促も飛ばないので、その状態は見えているほうがいい。カードの高さは期限の有無で変えない。
...
- 見た目。「見た目（決定）」のうち成立しているのは「感情は面、それ以外の意味は線」だけ。4つの画面はナビゲーションタイトルに役割名を出し、アクセント色は標準のまま、呼び名も管理者・プレイヤーのまま。ビジョンは `Form` の一節に置かれていて、感情ボタンは行の中の小さな要素。ヒートマップは管理者側にしか出ていない（プレイヤーのタスク行に背景色がない）。
