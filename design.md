...
- 保存は、もとにした状態からサーバーの状態が変わっていないときだけ通す。変わっていたら、取り直した最新の状態へ同じ操作を当て直して保存し直す。
  - 集約を丸ごと1レコードに書くので、後から書いた側を勝たせると、相手が別の項目に加えた変更まで巻き戻す（感情を付けた直後に承認されると、感情が消える）。
  - 当て直すのは操作で、状態どうしのマージはしない。相手が先に済ませていて操作が成り立たなくなったら、ドメインの拒否としてそのまま返す。
...
- **相手の変更は、プッシュで起こされた側が取り直す。** 同期層から変更を配るストリームは持たない。配る形にすると、プッシュを受けたアプリ側は取り直しが終わったかを知れず、バックグラウンドの実行時間を返す判断ができない。
  - プッシュだけには頼らない。サイレントプッシュは間引かれることがあり、アプリを終了させている間は届かない。前面に戻ったときと、画面を引っ張ったときにも取り直す。
  - 取り直しの失敗を知らせるのは、画面を引っ張ったときだけ。頼まれていない取り直しの失敗でアラートを出すと、圏外のまま前面に戻るたびに操作を止めることになる。
...
- ライフサイクル: `draft`（起案中）→ `proposed`（承認待ち）→ `active`（確定・高々1個）→ `achieved` / `abandoned`（履歴）
  ...
  - `draft` の間は、プレイヤーが中身を書き直せるし、取り下げられる。取り下げた起案は記録に残さない。合意に至っていないので、2人で歩んできた記録には当たらない。
  - `proposed` になったら書き直しも取り下げもできない。提出したあとは管理者の番で、承認するかを決めている間に、見ている中身が変わったり消えたりしないようにする。引っ込めたいときは差し戻しを待つ。
...
- 閉じたビジョンの履歴 + 感情リアクションの履歴 = **2人で歩んできた記録**として積み上がる。
- 記録は起案の新しい順に並べる。閉じた順にするには閉じた時刻をモデルに足すことになり、同期して両者で一致させる項目が1つ増える。
...
- **根の4画面にはナビゲーションタイトルを置かない。** 役割名を消した跡に別の語を入れない。タブも階層もなく、常に4つのうち1画面しか出ないので、見出しは「いまどこにいるか」を教えない。
- **相手を指す語は残す。** 「管理者の承認を待っています」「プレイヤーの起案を待っています」のように、誰の番かを伝えている文には上の理由が当たらない。固定値の反復ではなく、消すと文が成り立たなくなる。
- **役割ごとに別のアクセント色を置く。** 管理者は藍、プレイヤーは濃い青緑。ロールは端末ごとに固定なので、片方の画面に出るのは常に同じ1色で、これだけでは見た人に自分の役割を教えない。2色が同じ場に並ぶのは、2人が端末を並べたときと役割を選ぶ画面だけで、色の役目はそこで別の側だと分かることにある。暖色を使わないのは、感情リアクションが赤・橙・緑を占めているため。役割色をそこへ寄せると、行の色が感情を指しているのか役割を指しているのか読めなくなる。寒色のうち systemTeal と systemCyan を使わないのは、白地に対して 2.6:1 しかないため。この色は tint を通してボタンの文字色になるので、本文として読める比が要る。濃い青緑は 5.6:1 で、藍の 5.7:1 と揃う。標準の systemBlue が出るボタンと同じ画面に並ぶので、青から離すことも要る。この色は明暗で2値を持たせる。藍はシステムカラーで暗い側では明るい方へ振れるため、1値のままにすると暗い背景でだけ藍と比が揃わなくなる。
...
- **画面の先頭はビジョン。** active なビジョンは高々1個という不変条件が設計の中心なので、タスクの追加欄と同じ重さの節に置かない。画面の先頭のカードにして残り日数を出し、タスクはその下に従属させる。スクロールしても画面に留めることはしない。カードは達成基準と残り日数のバーまで持って縦に大きいので、留めるとタスク一覧に使える高さがその分ずっと削られる。
- **ビジョンを載せるカードは、期限が無くても同じ場所に「期限なし」と出す。** 期限は任意だが、カードは節と違って出さないという選択がなく、空欄にすると設定し忘れなのか意図的なのかが画面から分からない。承認するか差し戻すかを決める場面では、期限を決めずに起案したのか決め忘れたのかが判断の材料になる。カードの高さは期限の有無で変えない。
- **感情ボタンはタスク行で一番押しやすいものにする。** プレイヤーの唯一の主体性なので、完了報告より小さいと重みが逆になる。
- **フォントは標準のまま。** 独自フォントを入れるとバンドルサイズが増える。
- **呼び名は「見届ける人」/「挑む人」。** 管理者・プレイヤーは、手段だけを握って目的は握らない、という境界を言い表せていない。**画面に残す語すべてに及ぶ** ── 役割を選ぶ画面と、相手を指す文の両方。片方だけ変えると、同じ相手を2つの名前で呼ぶことになる。及ぶのは画面に出す文字列だけで、型名（`Role.manager` / `.player`）もこの文書も管理者・プレイヤーで通す。この文書は型名を常に参照しているので、表示用の呼び名に置き換えると型との対応が読めなくなる。
...
- ビジョンの一文・達成基準と、タスクの名前は、空白だけにはできない。空白だけの動機は、書かなかったもの（nil）として持つ。どれも前後の空白と改行は落として持つ。残すと、空白だけかの判定に使った文字列と持つ値が食い違い、一文の上に空行が出たりする。
...
1. MCで何を渡すか（CKShare URL or 独自トークン）と、CloudKit Sharingの招待フローとの噛み合わせ
   ...
   -> **実行検証はこれから**: MC は実機同士でないと繋がらないので、この検証だけは実機2台が要る。
...
5. **ペアリング時のロール割り当て** -> 済。最初の画面では、ロールを選ぶか、相手の招待を受けるかを選ぶ。2台はつながったら、まず選んだものを MC で送り合い、その組み合わせで共有を作る側を決める。共有を作る側は自分のロールを入れた状態をルートレコードに書いてから共有し、CKShare の URL を MC で渡す。受ける側は MC でロールを受け取るのではなく、参加後に読み込んだ状態から `ownerRole` を読む。
   - 最初の画面は「どちらで使いますか」と聞くので、2人がそれぞれ自分のロールを選ぶことが多い。ロールが違えば、そのままペアにする。共有は見届ける人を選んだ側が作り、挑む人を選んだ側が受ける。片方が相手の招待を受けるを選んだときは、ロールを選んだ側が作る。
   - ロールが同じとき、2台とも「相手の招待を受ける」を選んだときは、共有を作る前に止めて、片方に選び直してもらう。
   - 共有を作る側の iCloud が容量を理由に作成を断ったら、もう片方が代わりに共有を作り、断られた側が受ける。`ownerRole` は共有を作った側のロールなので、代わりに作る側は相手のロールの反対を書く。どちらの端末のロールも、選んだとおりになる。代わりに作る側も失敗したら、断られた側には容量の案内を出す。
6. **片側リセット時の整合性** -> 決定。**リセットは片側だけでは終われない。必ず両者のパートナーシップが終わる。**
   ...
   - 共有を作った側は private DB のゾーンごと消す。受けた側は共有されたルートレコードを消す。共有ゾーンを消しても抜けるのは自分だけで、共有を作った側のレコードは残るため。
   - 相手側は次の取得でレコードが無くなっているので、`pairing` が nil の状態になり最初の画面に戻る。**終了フラグは状態に足さない。** レコードの不在がそのまま「もう無い」を表しているので、増やすと同じことを2通りで表すことになる。
   - 入り口は設定画面の一番下の「ペアリングをやり直す」に置き、4つのロール画面の左上には設定画面への歯車を置く。ふだんの操作と同じ場所に、すべてを消す操作を置かないため。破壊的なので確認を挟む。
...
- 催促の判定 ── 期限と滞留から誰を催促するかを決める純粋関数（`Nudge`）。判定に使う時刻は引数で渡す。まだ始まっていない催促は始まる時刻も返し、端末の通知として予約する。誰も操作しないまま期限が過ぎても、予約した時刻に鳴る。
- タスク一覧は両側とも、行の背景を感情の色で塗る（絵文字と併記。色だけに意味を持たせない）。
...
- 通信の失敗は、利用者が自分で手を打てる単位に分けて見せ、元のエラーは画面に出さずログ（`Logger`、サブシステム `com.fujimori.PairCommit`）に残す。error レベルは端末に保存されるので、あとから Mac につないで Console.app で読める。
- ペアリング結果（ルートレコードのIDと、共有を作った側かどうか）は端末に残し、起動し直したら前回の相手につなぎ直す。つながらないだけならペアは残して再試行を出し、ペアが終わっていたときだけ役割の選択に戻す。
- 相手の変更を受け取る経路 ── サイレントプッシュで起きて状態を取り直し、催促を通知として出すところまでを画面の外で行う。

**まだないもの**:

- 予約した催促の取り下げ。予約した通知の文面は予約した時点の状態で作り、配信済みの通知を時刻で取り下げる手段もないので、相手の変更がこの端末に届かないまま時刻が来ると、次にアプリが開かれるかプッシュで起きるまで、もう解消した催促が鳴って通知センターに残る。プッシュの配送は保証されないので、この制約は残る。
  - 時刻が進むだけで解消する催促（期限が近い → 期限切れ）も通知センターに残る。期限が近い催促の文面は期限の日付を言うだけにしてあるので、期限を過ぎて残っていても嘘にはならない。同じタスクの2つの催促で通知の識別子を共有して置き換える形は取れない。どちらもまだ始まっていないうちは両方を予約する必要があり、同じ識別子の予約はあとから足したほうで置き換わる。
- ウィジェット用の App Group の capability。
- ログを端末から書き出す手段。Mac につなげない相手の端末のログは、sysdiagnose を取ってもらうしかない。
- CloudKit のスキーマの本番環境への反映。開発環境で作られたレコード型は、CloudKit Console で本番へ反映するまで、App Store や TestFlight から入れたアプリでは使えない。

**保留**: 実機での検証。MC はシミュレータ同士では繋がらないので、ペアリングから先は実機2台でしか動かせない。1台だけで使う入り口（`LocalPairing`）は、ペアリングを本物にしたときに落とした。
