...
- 予約した催促の取り下げ。予約した通知の文面は予約した時点の状態で作り、配信済みの通知を時刻で取り下げる手段もないので、次にアプリが開かれるかプッシュで起きるまで、もう解消した催促が鳴って通知センターに残る。相手の変更がこの端末に届かないまま時刻が来た場合だけでなく、期限が近いという催促が期限切れの催促に変わるときのように、時刻が進むだけで解消する場合にも起きる。プッシュの配送は保証されないので、署名と capability が入ったあとも残る。
