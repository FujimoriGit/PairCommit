...
- **役割ごとに別のアクセント色を置く。** 管理者は藍、プレイヤーは濃い青緑。ロールは端末ごとに固定なので、片方の画面に出るのは常に同じ1色で、これだけでは見た人に自分の役割を教えない。2色が同じ場に並ぶのは、2人が端末を並べたときと役割を選ぶ画面だけで、色の役目はそこで別の側だと分かることにある。暖色を使わないのは、感情リアクションが赤・橙・緑を占めているため。役割色をそこへ寄せると、行の色が感情を指しているのか役割を指しているのか読めなくなる。寒色のうち systemTeal と systemCyan を使わないのは、白地に対して 2.6:1 しかないため。この色は tint を通してボタンの文字色になるので、本文として読める比が要る。濃い青緑は 5.6:1 で、藍の 5.7:1 と揃う。標準の systemBlue が出るボタンと同じ画面に並ぶので、青から離すことも要る。この色は明暗で2値を持たせる。藍はシステムカラーで暗い側では明るい方へ振れるため、1値のままにすると暗い背景でだけ藍と比が揃わなくなる。役割が決まる前の画面（役割の選択・ペアリング・つなぎ直し）はどちらの色も使えないので、既定の色に無彩色を置く。ここを標準のままにすると、2つの役割色を並べて見せる画面に、どちらの側も指さない systemBlue が3色目として出る。
- **感情の色は変えない。** 変えると、積み上がった履歴の読み方まで変わる。
- **感情は面、それ以外の意味は線で出す。** 赤・橙・緑は感情のほかにも、エラー・催促・達成基準の下読みで使っている。感情だけは行の背景（面）で出し、他は文字とアイコン（前景）で出す。色相が重なっても層が違えば意味は混ざらないので、暖色を意味ごとに割り当て直さずに済む。線に使う赤・橙・緑は、システムカラーのままでは白地に対して 2.2〜3.6:1 で、本文として読める 4.5:1 に届かない。明るい外観では、線に使うときだけ暗い値に替える。
