...
enum NudgeNotifications {
    ...
    static func post(for role: Role, in state: PartnershipState, now: Date = .now) async {
        ...
        for (id, nudge) in wanted where !delivered.contains(id) {
            try? await center.add(request(id, nudge, in: state, trigger: nil))
        }
        ...
        for (nudge, startsAt) in state.upcomingNudges(for: role, now: now) {
            let trigger = UNTimeIntervalNotificationTrigger(timeInterval: startsAt.timeIntervalSince(now), repeats: false)
            try? await center.add(request(identifier(of: nudge), nudge, in: state, trigger: trigger))
        }
    }
